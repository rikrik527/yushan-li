...
beastBtn.insertAdjacentHTML(
  'afterbegin',
  "<div class='beast-lowbox'><div class='beast-name'>The Beast</div><ul class='beast-status'><li class='beast-info'></li></ul></div>",
);
magicBtn.insertAdjacentHTML(
  'afterbegin',
  "<div class='magic-lowbox'><div class='magic-name'>Debbie</div></div><ul class='magic-status'><li class='magic-info'></li></ul></div>",
);
samuraiBtn.insertAdjacentHTML(
  'afterbegin',
  "<div class='samurai-lowbox'><div class='samurai-name'>Samurai魂</div></div><ul class='samurai-status'><li class='samurai-info'></li></ul></div>",
);

crazyBtn.insertAdjacentHTML(
  'afterbegin',
  "<div class='crazy-lowbox'><div class='crazy-name'>Mr.Mayamoto</div><ul class='crazy-status'><li class='crazy-info'></li></ul></div>",
);
nopoBtn.insertAdjacentHTML(
  'afterbegin',
  "<div class='nopo-lowbox'><div class='nopo-name'>Nopo</div><ul class='nopo-status'><li class='nopo-info'></li></ul></div>",
);
shanBtn.insertAdjacentHTML(
  'afterbegin',
  "<div class='shan-lowbox'><div class='shan-name'>Shan-Li</div><ul class='shan-status'><li class='shan-info'></li></ul></div>",
);
...
mainBc.insertAdjacentHTML(
  'afterbegin',
  '<div class="background"></div><div class="background2"></div>',
);
...
module.exports.btnNext = function() {
  ...
  var beastCount = 0,
    ...
    magicCount = 0;
  var btn = obj.getAll(
    '.beast-btn,.crazy-btn,.shan-btn,.judo-btn,.nopo-btn,.samurai-btn,.magic-btn',
  );
...
function linkedList() {
  var Node = function(element) {
    ...
    this.next = null;
    console.log(
      'var node = function(element){ this.element = element} this.element is',
      this.element,
    );
  };
  var length = 0;
  ...
  this.append = function(element) {
    var node = new Node(element),
      current;
    console.log(
      'var node = new Node(element),current;node and current are',
      node,
      current,
    );
    if (head === null) {
      ...
      current.next = node;
      console.log(
        'current.next = node找到最後一項 將其next賦予node,建立連結 ',
        current.next,
      );
    }
    length++;
    ...
    if (position > -1 && position < length) {
      ...
      if (position === 0) {
        ...
        while (index++ < position) {
          ...
          console.log(
            'else while index++ < position = previous = current',
            previous,
            'current = current.next',
            current,
            '將previous與current的下一項連結起來 跳過current 進而移除他 previous.next = current.next',
          );
        }
      }
      length--;
      ...
      console.log(
        'length-- from method this.removeAt = function',
        length,
        'return current.element',
        current.element,
      );
